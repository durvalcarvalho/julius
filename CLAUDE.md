...
**O que ainda não foi medido, de propósito** (ver "Fora do escopo" para a lista completa de recusas): os cortes `NARRATE_FULL_MAX_RECORDS = 6`/`NARRATE_FULL_MAX_GROUPS = 3` são chute, a confirmar contra o catálogo real; a qualidade da voz (soa como o personagem ou como um chatbot educado?) é leitura humana, sem métrica; nenhuma chamada real ao `deepseek-flash` passou pelo prompt `persona` ainda. Isso vai para `docs/como-testar-o-bot.md`, passos 16–20 (dono: o usuário).
