...
**Rodada real da v2.7** (18/09/2026, contra cópia do banco de produção — 105 produtos, 132 preços, 5 mercados, 7 chamadas `persona`, **US$ 0,0014**). Mediu o que a rodada anterior só chutava: `NARRATE_FULL_MAX_RECORDS = 6` cobre **69 das 72 buscas possíveis** (96%, uma palavra por produto, até 50 resultados); o único `mercados comparar` real do catálogo tem **exatamente 6 grupos**, e a IA narrou os 6 inteiros, grounded (nenhum valor fora dos fatos), sem a guarda rejeitar nada — `NARRATE_FULL_MAX_GROUPS` subiu de 3 (chute) para **6** (medido). O caso de 12 registros (mais temperos do catálogo) confirmou o Modo B funcionando como desenhado: a IA resumiu só o mais barato e o mais caro, a tabela ficou intacta embaixo. Tom lido como funcional — direto, cita preço, fecha com uma frase seca ("Quem paga mais é bobo.", "Sem valor, não tem conversa.") — mas mais comedido que o character bible completo (sem as comparações "isso dá pra comprar X" do personagem); não ajustado o prompt por isso, porque essa retórica exigiria uma guarda nova contra objeto inventado que este projeto não tem, e o texto atual já passa na guarda de dinheiro sem folga. **O que essa rodada não cobre**: os botões, a edição de mensagem e o roteiro dentro do Telegram de verdade (passos 1–15 e 19 de `docs/como-testar-o-bot.md`) — só a chamada a `narrate()` foi exercitada diretamente, fora do `python-telegram-bot`. Isso continua sendo do usuário.
