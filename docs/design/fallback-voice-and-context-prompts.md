...
2. **Caso 3 falhou na primeira medição — corrigido em seguida por `/sc:troubleshoot --fix` (mesma sessão).** Causa raiz real: nem a regra de prompt nem o teste original usavam o sinal mais forte do incidente que abriu este caso — o **mesmo preço repetido** ("coca por 12 reais" → "refrigerante 2L por 12 reais"), não só a variação de tamanho sozinha. Sem repetir o preço, "refrigerante 2 litros" tem resposta própria de verdade no catálogo (Pepsi 2L existe) — buscar direto não era claramente errado, só não testava o caso real. Corrigido: a regra do `SYSTEM_PROMPT` (v4) passou a citar "preço repetido pra um produto parecido" como o sinal mais forte de mesma compra; o teste foi reescrito repetindo o preço nos dois turnos. Segunda medição: **passou**, e o modelo citou o próprio sinal na resposta — *"O preço é o mesmo (R$ 12) e o item é bem parecido com a 'coca' que você citou agora há pouco [...] Só me confirma: é o mesmo item da mensagem anterior ou é outro produto?"*. Suíte `real_ai` completa depois da correção: **15 de 15**, US$ 0,04 na rodada.
