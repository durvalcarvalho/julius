...
from julius.domain.models import Hint, ImportResult
...
def test_after_import_flags_package_size_only_for_new_products_without_content(conn):
    ...
    (hint,) = guidance.after_import(conn, result)
    assert hint.kind == "PACKAGE_SIZE_IN_DESCRIPTION"
    ...
    (again,) = guidance.after_import(conn, result)
